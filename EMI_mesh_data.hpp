...
void writeToMatlabPath_matlab(NumaBCRSMatrix<Entry,Index> const& A, Dune::BlockVector<VEntry> const& b, std::string const& basename, std::string const& path, bool gen_rhs, int precision=16)
{
...
template<class VariableSet, class creator_type, class Matrix, class Vector, class Functional, class CellFilter, class Assembler>
void construct_A_submatrix(int subIdx, 
                           VariableSet const& variableSet, 
                           typename VariableSet::VariableSet u,
                           std::vector<int> &sequenceOfTags, creator_type &creator, 
                           std::vector<Matrix> &subMatrices,
                           std::vector<Matrix> &subMatrices_M,
                           std::vector<Matrix> &subMatrices_K,
                           Matrix A_,
                                                          ...
                                                          std::vector<Matrix> &subMatrices_K)
{
  ...
  // std::mutex mapMutex_original; 
  // for (size_t t = 0; t < numThreads; ++t) {
  //   threads_original.emplace_back([t, n, numThreads,&variableSet, &u, &sequenceOfTags, &creator, &subMatrices, &subMatrices_M, &subMatrices_K,
  //                                  &map_GammaNbr, &sequenceOfsubdomains, &F, &dt, &Cellfltr, &cells_set, &assembler,&map_indices, &tags, &map_II, &map_GammaGamma, 
  //                                  &assemblyThreads, &A_,&K_, &M_, &map_GammaGamma, &assemblyThreads, &i2Tag, &Matrix_mass_petsc, &Matrix_stiffness_petsc, &write_to_file, &matlab_dir, &Fs_petcs, &mapMutex_original]() {
  ...
  //         std::lock_guard<std::mutex> lock(mapMutex_original);
  //         {
  //           Cellfltr.set_tags(tags_target);
  //           Cellfltr.select_based_on_tag(true);
  ...
  //       // writeToMatlabPath(subMatrix_mass,Fs_petcs_sub,"resultBDDC_mass"+path,matlab_dir, false);
  //       // writeToMatlabPath(subMatrix_stiffness,Fs_petcs_sub,"resultBDDC_stiff"+path,matlab_dir, false);
  //       // //construct_A_submatrix(subIdx, variableSet, u, sequenceOfTags, creator, subMatrices, subMatrices_M, subMatrices_K, A_, K_, M_, map_GammaNbr, sequenceOfsubdomains, F, dt, Cellfltr, cells_set, assembler, map_indices, tags, map_II, map_GammaGamma, assemblyThreads, i2Tag, Matrix_mass_petsc, Matrix_stiffness_petsc, write_to_file, matlab_dir);
  //     }
  //   });
  // }

  // // Wait for all threads to finish
  // for (auto& thread : threads_original) {
  //   if (thread.joinable()) {
  //       thread.join();
  //   }
  // }
...
bool isSubset(const std::vector<int>& V, const std::vector<int>& W) {
    // Convert W to an unordered_set for fast lookups
    std::unordered_set<int> wSet(W.begin(), W.end());

    // Check each element of V to see if it's in W
    for (int v : V) {
        if (wSet.find(v) == wSet.end()) {
            return false; // Element of V not found in W
        }
    }
...
void computeSharedDofs(int subIdx, const std::vector<int>& sequenceOfTags, 
  ...
  int tag = sequenceOfTags[subIdx];
  //std::vector<int> tmp = sequanceOfsubdomainsKaskade.at(tag); // Access with at() for safety
  std::vector<int> tmp;
  {
    std::lock_guard<std::mutex> lock(mapMutex);
    tmp = sequanceOfsubdomainsKaskade.at(tag);
  }

  for (int i = 0; i < tmp.size(); ++i) {
      int key = tmp[i];
      {
        std::lock_guard<std::mutex> lock(mapMutex); // Lock during map access
        auto it = MapSharedDofsKaskadeTuple.find(key);

        if (it != MapSharedDofsKaskadeTuple.end()) {
            it->second.push_back(i3tuple(subIdx, i, key));
        } else {
            MapSharedDofsKaskadeTuple[key] = {i3tuple(subIdx, i, key)};
        }
      }
  }
}
...
template<class Matrix, class Vector>
void computeAsFs( int subIdx, const std::vector<int>& sequenceOfTags,  
                  std::map<int,int> & T2Index, std::set<int> & arr_extra_set,
                  std::vector<Matrix> &subMatrices,
                  std::vector<Matrix> &subMatrices_M,
                  std::vector<Matrix> &subMatrices_K,
                  std::map<int,std::set<int>> &map_II,
                  ...
                  std::mutex& mapMutex)
{
  ...
  {
    std::lock_guard<std::mutex> lock(mapMutex);
    // Ensure the key exists before access
    if (sequanceOfsubdomainsKaskade.find(tag) == sequanceOfsubdomainsKaskade.end()) {
      sequanceOfsubdomainsKaskade[tag] = {};
    }
  }
...
template<class Matrix, class Vector>
void construct_As_parallel(std::vector<int> arr_extra, 
                  std::vector<int> sequenceOfTags, 
                  std::map<int,std::set<int>> map_II,
                  std::map<int,std::set<int>> map_GammaGamma,
                  std::map<int,std::set<int>> map_GammaGamma_noDuplicate,
                  ...
                  std::map<int,int> & T2Index)
{
  ...
  std::mutex mapMutex; 
  std::map<int,std::vector<int>> sequanceOfsubdomainsKaskade;
  // Initialize the map to ensure all keys exist
  {
    std::lock_guard<std::mutex> lock(mapMutex);
    for (int tag : sequenceOfTags) {
      if (sequanceOfsubdomainsKaskade.find(tag) == sequanceOfsubdomainsKaskade.end()) {
        sequanceOfsubdomainsKaskade[tag] = {};
      }
    }
  ...
  for (int t = 0; t < numThreads; ++t) {
    threads.emplace_back([t, n, numThreads, 
                          &sequenceOfTags,  
                          &T2Index, 
                          &arr_extra_set,
                          &subMatrices,
                          ...
                          &subMatrices_kaskade,
                          &subMatrices_kaskade_Ms,
                          &subMatrices_kaskade_Ks,
                          &sequanceOfsubdomainsKaskade,
                          &weights,
                          &Fs,
                          &rhs_kaskade,
                          &map_indices,
                          &IG_seq,
                          &mapMutex]() {
      for (int subIdx = t; subIdx < n; subIdx += numThreads) {
  ...
  std::vector<std::thread> threads_sharedDofs;
  typedef std::tuple<int,int,int> i3tuple;
  std::map<int, std::vector<i3tuple>> MapSharedDofsKaskadeTuple;

  std::mutex mapMutex_shared; // Mutex for thread-safe access to the shared map
  // Divide work among threads
  for (int t = 0; t < numThreads; ++t) {
    threads_sharedDofs.emplace_back([t, n, numThreads, &sequenceOfTags, &sequanceOfsubdomainsKaskade, &MapSharedDofsKaskadeTuple, &mapMutex]() {
      for (int subIdx = t; subIdx < n; subIdx += numThreads) {
        computeSharedDofs(subIdx, sequenceOfTags, sequanceOfsubdomainsKaskade, MapSharedDofsKaskadeTuple, mapMutex);
      }
    });
  }

  // Wait for all threads to finish
  for (auto& thread : threads_sharedDofs) {
    if (thread.joinable()) {
        thread.join();
    }
  }
...
void merge_inner_interface_bddc_fused(
  std::vector<int> sequenceOfTags_extra,
  std::map<int,int> T2Index,
  std::map<int,std::set<int>> map_II,
  std::map<int,std::set<int>> map_GammaGamma,
  ...
  std::map<int,std::set<int>> & map_GammaNbr_Nbr_noDuplicate_fused,
  std::map<int, int> & map_t2l_fused,
  std::vector<Matrix> & subMatrices_fused){

    for (int subInx = 0; subInx < sequenceOfTags_extra.size(); ++subInx)
  {
    int tag = sequenceOfTags_extra[subInx];
    int index_extra = T2Index[tag];
    int index_intra = T2Index[tag+1];
    subMatrices_fused[subInx] = subMatrices[index_extra];
    subMatrices_fused[subInx] += subMatrices[index_intra];

    std::set<int> II_extra = map_II[tag];
    std::set<int> II_intra = map_II[tag+1];
    II_extra.insert(II_intra.begin(), II_intra.end());

    std::set<int> GammaGamma_extra = map_GammaGamma[tag];
    std::set<int> GammaGamma_intra = map_GammaGamma[tag+1];
    GammaGamma_extra.insert(GammaGamma_intra.begin(), GammaGamma_intra.end());

    std::set<int> GammaGamma_noDuplicate_extra = map_GammaGamma_noDuplicate[tag];
    std::set<int> GammaGamma_noDuplicate_intra = map_GammaGamma_noDuplicate[tag+1];
    GammaGamma_noDuplicate_extra.insert(GammaGamma_noDuplicate_intra.begin(), GammaGamma_noDuplicate_intra.end());

    std::set<int> GammaNbr_Nbr_noDuplicate_extra = map_GammaNbr_Nbr_noDuplicate[tag];
    std::set<int> GammaNbr_Nbr_noDuplicate_intra = map_GammaNbr_Nbr_noDuplicate[tag+1];
    GammaNbr_Nbr_noDuplicate_extra.insert(GammaNbr_Nbr_noDuplicate_intra.begin(), GammaNbr_Nbr_noDuplicate_intra.end());

    map_II_fused[tag] = II_extra;
    map_GammaGamma_fused[tag] = GammaGamma_extra;
    map_GammaGamma_noDuplicate_fused[tag] = GammaGamma_noDuplicate_extra;
    map_GammaNbr_Nbr_noDuplicate_fused[tag] = GammaNbr_Nbr_noDuplicate_extra;

    map_t2l_fused[tag] = II_extra.size() + GammaGamma_extra.size();

    std::vector<int> I(II_extra.begin(), II_extra.end());
    std::vector<int> gamma(GammaGamma_extra.begin(), GammaGamma_extra.end());
    std::vector<int> gamma_noDup(GammaGamma_noDuplicate_extra.begin(), GammaGamma_noDuplicate_extra.end());
    std::vector<int> gammaNbr_noDup(GammaNbr_Nbr_noDuplicate_extra.begin(), GammaNbr_Nbr_noDuplicate_extra.end());

    std::cout << subInx << " tag: "<< tag << ": I.size() " << I.size()<< ": gamma.size() " << gamma.size()<< ": gamma_noDup.size() " << gamma_noDup.size()<< ": gammaNbr_noDup.size() " << gammaNbr_noDup.size() <<std::endl;
  }

}

template<class Matrix, class Vector>
void construct_As_fused(std::vector<int> arr_extra, 
                  std::vector<int> sequenceOfTags, 
                  std::map<int,std::set<int>> map_II,
                  std::map<int,std::set<int>> map_GammaGamma,
                  std::map<int,std::set<int>> map_GammaGamma_noDuplicate,

                                                          ...
                                                          double R_extra)
{
  ...
  // Divide the subdomains among threads
  {
    ...
    std::vector<std::future<void>> futures;
    int nsub = sequenceOfTags.size();
    int chunkSize = (nsub + numThreads - 1) / numThreads; // Calculate chunk size (ceil(n/m))

    for (size_t threadIdx = 0; threadIdx < numThreads; ++threadIdx) {
      int startIdx = threadIdx * chunkSize;
      int endIdx = std::min(startIdx + chunkSize, nsub);

    futures.push_back(std::async(std::launch::async, [startIdx, endIdx, &sequenceOfTags, &creator, &subMatrices, &A_petsc, &rhs_petsc_test, &Fs_petcs, &weights]() {
                    for (int subIdx = startIdx; subIdx < endIdx; ++subIdx) {
                      Matrix subMatrix(creator);
                      subMatrix = subMatrices[subIdx];

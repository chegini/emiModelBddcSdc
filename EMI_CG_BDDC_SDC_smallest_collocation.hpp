...
#ifndef INTEGRATE_CG_BDDC_SDC_SMALLEST_COLL_HH
#define INTEGRATE_CG_BDDC_SDC_SMALLEST_COLL_HH

using namespace Kaskade;

template <class Interfaces, class Matrix, class Vectors, class Vector, class ReactionDerivatives>
typename Matrix::field_type sdcIterationStepBDDC_smallest_collocation(bool BDDC_SDC_with_initial, bool BDDC_verbose, bool BDDC_SDC_verbose, int sweep, bool cg_solver, int iter_cg_with_bddc, double tol, std::string matlab_dir, Interfaces const& interfaces,int n_subdomains ,SDCTimeGrid const& grid, SDCTimeGrid::RealMatrix const& Shat,
...
void computeRHS_BDDC_smallest_collocation(int step, 
                State const& x,
                CellFilter & Cellfltr,
                int number_cells,
...
typename VariableSet::VariableSet semiImplicit_CG_BDDC_SDC_smallest_collocation( GridManager<Grid>& gridManager,
                                                            Equation& eq,
                                                            CellFilter & Cellfltr,  
                                                            ...
                                                            bool BDDC_SDC_verbose)
{
  ...
  for (steps=0; !done && steps<maxSteps; ++steps) // maxSteps
  {
    ...
    do
    {
      ...
      computeRHS_BDDC_smallest_collocation(steps,x,
                      Cellfltr,
                      number_cells,
                      collocationU,
                      size_e,
      ...
      sweepNorm_bddc.push_back( sdcIterationStepBDDC_smallest_collocation(BDDC_SDC_with_initial, BDDC_verbose, BDDC_SDC_verbose, sweep, cg_solver,iter_cg_with_bddc,tol,matlab_dir,
                                                            ifa,
                                                            n_subdomains,
                                                            grid,
                                                            Shat,
  ...
  writeVectorTofile(x,"matlab_dir/emiSDCBDDCLastSmallest");  
 return x;
}
